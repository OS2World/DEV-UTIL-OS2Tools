...
/* Remark:
 - extents vorher irgendwie ermitteln
 - altes file austauschen, wenn neues weniger (gleich viele ?) extents hat!
 - schnellere kopierroutine

 Momentan sollte das Programm vielmehr FRAG als DeFRAG heissen !
 */
...
void help (void)
{
  TOOLVERSION("Defrag",                                 /* application name */
              ...
              "I'm currently rather named sluggish FRAG instead"  /* Remark */
              "of DEFRAG.",
...
printf("\nDEBUG: DosOpenSource(%s)=%u.",pszPath,rc);
  ...
  hfFileOutput = 0;                                       /* no valid handle */
  do
  {
    ...
    if (hfFileOutput != 0) /* previously assigned handle ? Don't loose them !*/
      DosClose(hfFileOutput);
  ...
  if (rc != NO_ERROR)                                    /* check for errors */
  {
    DosDelete(szFileNameBuffer);                /* delete the temporary file */
    return (rc);                               /* then raise error condition */
  }
