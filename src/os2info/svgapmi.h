 ...
 *   10/08/96  @V3.2TSU03 169905  Cirrus Logic OS/2 FS save/restore problem
 *
 *****************************************************************************/
...
/*
** GPMIMODEINFO is required for the GENPMI<->pmiextension mechanism to allow
** for enhanced PMI support based on the generic VESA bios support provided by
** GENPMI.  
*/ 
typedef struct _GPMIMODEINFO {
   VIDEOMODEINFO  ModeInfo;
   ULONG          ulBankRoutine;
   /*
    * There are three kinds of window systems.
    *   1) Single window system: WinASegment will be used as readable &
    *      writeable.
    *   2) Overlapping dual window system: WinASegment and WinBSegment share
    *      the same physical address, but WinASegment is writeable and
    *      WinBSegment is readable. Therefore, read and write have to go
    *      through different window.
    *   3) Nonoverlapping dual window system: WinASegment and WinBSegment are
    *      of different addresses. Both of them are readable and writeable.
    *
    *   In this genpmi.dll, we support all of them. For 2), we have to
    *   set the bank on both WinASegment and WinBSegment.
    *   For 3), we will use only WinASegment.
    */
   #define  SINGLE_WINDOW            0
   #define  OVERLAP_DUAL_WINDOW      1
   #define  NONOVERLAP_DUAL_WINDOW   2

   ULONG          ulBankingSystem;
   ULONG          ulShiftLeftBits;
} GPMIMODEINFO, FAR *PGPMIMODEINFO;

/*
** Prototype for GPMIMapVRAM helper function provided by GENPMI.  If you are
** going to use this function then import GENPMI.GPMIMapVRAM in your dlls
** def file.
*/
ULONG  GPMIMapVRAM(ULONG ulPhysAddr, ULONG ulLength, PBYTE *pVRAM);
...
#define  PMI_MODE_LINEAR_APERTURE 0x100 /* linear aperture mode */
